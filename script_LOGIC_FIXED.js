...
const ROOM_LOGIC = {
    ...
    // --- INSTITUTIONAL / CIVIC ---
    "Classroom": { tags: ["Clean", "Learning"], link: ["Hallway", "Library Archives"], avoid: ["Industrial", "Hazard", "Bedroom"] },
    "Gymnasium": { tags: ["Large", "Rec"], link: ["Locker Room", "Cafeteria"], avoid: ["Office", "Quiet"] },
    "Library Archives": { tags: ["Quiet", "Paper"], link: ["Study Hall", "Admin Office"], avoid: ["Water", "Fire", "Loud"] },
    "Principal's Office": { tags: ["Office", "Admin"], link: ["Admin Office", "Hallway"], avoid: ["Classroom"] },
    "Main Hall": { tags: ["Hub", "Clean"], link: ["Classroom", "Admin Office", "Gymnasium"], avoid: ["Dirty"] },
    "Auditorium": { tags: ["Large", "Social"], link: ["Main Hall", "Hallway"], avoid: ["Bedroom", "Industrial"] },
    "Faculty Lounge": { tags: ["Office", "Clean"], link: ["Classroom", "Admin Office"], avoid: ["Student"] },

    // --- LOGIC FIXES FOR EXISTING ROOMS ---
    "Kitchen": { tags: ["Service", "Loud"], link: ["Cafeteria", "Dining Hall", "Cold Storage", "Pantry"], avoid: ["Bedroom", "Toilet", "Morgue", "Office", "Restroom"] },
    "Cafeteria": { tags: ["Food", "Social"], link: ["Kitchen", "Hallway"], avoid: ["Restroom", "Morgue", "Toxic Pit", "Toilet"] },
    "Restroom": { tags: ["Water", "Private"], link: ["Hallway"], avoid: ["Kitchen", "Cafeteria", "Chapel", "Food"] },
    "Boiler Room": { tags: ["Industrial", "Dirty"], link: ["Janitor Closet", "Basement"], avoid: ["Nursery", "Executive Suite", "Sterile", "Clean"] }
};
...
function generateInterior(key, name) {
    ...
    if (config.mapType === 'ruins' && currentLevelIndex < 0) {
        // Check if the parent building was Institutional or Commercial first
        // We want "Basement Storage" or "Archives", not just Sewers
        const parentArch = getArchetype(name);

        if (parentArch === 'INSTITUTIONAL' || parentArch === 'COMMERCIAL') {
             // 70% chance of just being a spooky basement/archive, 30% chance of something weird
             if (Math.random() < 0.7) {
                 effectiveArchKey = 'GENERIC'; // Becomes storage/utility
             } else {
                 effectiveArchKey = (Math.random() < 0.5) ? 'SEWER' : 'CULT';
             }
        } else {
            // Standard logic for other building types (Hospitals, Police, etc.)
            effectiveArchKey = (Math.random() < 0.4) ? 'SEWER' : (Math.random() < 0.6) ? 'BUNKER' : 'CULT';
        }
        log(`SUB-LEVEL THEME: FORCED [${effectiveArchKey}] (Parent: ${parentArch})`, 'var(--pip-amber)');
...
function generateRuins(data, density, anchors) {
    ...
    if (currentLevelIndex < 0) {
        ...
        if (anchors.length > 0 && anchors[0].upperName) {
            // Ask the Master Archetype list what this is
            const arch = getArchetype(anchors[0].upperName);

            // Map the Archetype to your visual themes (SUB_THEMES)
            if (arch === 'MEDICAL' || arch === 'POLICE' || arch === 'CULT') {
                selectedThemeKey = 'creepier'; 
                foundSmartTheme = true;
            } else if (arch === 'INDUSTRIAL' || arch === 'BUNKER' || arch === 'SEWER') {
                selectedThemeKey = 'industrial';
                foundSmartTheme = true;
            } else if (arch === 'RETAIL' || arch === 'COMMERCIAL') {
                selectedThemeKey = 'residential'; // Uses apartments/lobbies/boilers
                foundSmartTheme = true;
            } else if (arch === 'INSTITUTIONAL') {
                // Special Override for Schools/Civic to avoid "Apartment Lobbies"
                // We inject specific room names for this run
                if (!SUB_THEMES['institutional']) {
                    SUB_THEMES['institutional'] = ["Classroom", "Library Row", "Cafeteria", "Boiler Room", "Study Hall", "Office"];
                }
                selectedThemeKey = 'institutional';
                foundSmartTheme = true;
            } else if (arch === 'ENTERTAINMENT' || arch === 'RESIDENTIAL') {
                selectedThemeKey = 'residential';
                foundSmartTheme = true;
            }
        }
        if (!foundSmartTheme) {
