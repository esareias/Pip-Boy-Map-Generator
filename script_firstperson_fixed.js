...
let playerAngle = 0; // Rotation in radians
let playerSpeed = 2; // Movement speed (pixels per frame)
let rotationSpeed = 0.05;
let fov = Math.PI / 3; // 60 degree FOV
let rayCount = 160; // Ray resolution (lower for better performance)
...
// Global V key to toggle view mode
window.addEventListener('keydown', (e) => {
...
function toggleFirstPersonMode() {
  ...
  if (firstPersonMode) {
    ...
    // Make sure we have a player token
    const player = tokens.find(t => t.name !== 'OVERSEER_TOKEN_ID') || tokens[0];
    if (!player) {
      log('WARNING: NO PLAYER TOKEN. DEPLOY A UNIT FIRST.', 'var(--pip-amber)');
    }
  } else {
...
function updateFirstPersonMovement() {
  ...
  if (moveX !== 0 || moveY !== 0) {
    ...
    if (gridX >= 0 && gridX < data.grid.length && 
        gridY >= 0 && gridY < data.grid[0].length) {
      // Check if walkable (1 = floor, 0 = wall)
...
function castRay(data, startX, startY, angle) {
  ...
  const stepSize = 0.05; // Smaller = more accurate but slower
  const dx = Math.cos(angle) * stepSize;
  const dy = Math.sin(angle) * stepSize;
  let distance = 0;
  const maxDistance = 40; // Maximum view distance in grid units
...
function drawFirstPerson() {
  ...
  if (!data || !data.grid) {
    ...
    const msg = 'NO MAP DATA - PRESS [SCAN LEVEL] FIRST';
  ...
  if (!player) {
    ...
    ctx.fillStyle = '#f00';
    ctx.font = 'bold 20px monospace';
    const msg = 'NO PLAYER TOKEN - DEPLOY A UNIT FIRST';
    ctx.fillText(msg, canvas.width/2 - 220, canvas.height/2);
    return;
  }

  // Screen dimensions (accounting for RENDER_SCALE)
  ...
  // Cast rays for each vertical strip
  ...
  for (let i = 0; i < rayCount; i++) {
    ...
    if (hit.hit) {
      ...
      const brightness = Math.max(20, Math.min(255, 255 - (correctedDistance * 10)));

      // Different colors based on map type
      let wallColor;
      if (config.mapType === 'vault') {
        wallColor = `rgb(${brightness * 0.3}, ${brightness * 0.5}, ${brightness})`;
      } else if (config.mapType === 'cave') {
        wallColor = `rgb(${brightness * 0.5}, ${brightness * 0.4}, ${brightness * 0.3})`;
      } else {
        wallColor = `rgb(${brightness * 0.6}, ${brightness * 0.5}, ${brightness * 0.4})`;
      }
      ...
      // Add vertical scan lines for retro effect
      if (i % 3 === 0) {
        ctx.fillStyle = 'rgba(0, 0, 0, 0.15)';
        ctx.fillRect(
          i * stripWidth * RENDER_SCALE,
          wallTop * RENDER_SCALE,
          stripWidth * RENDER_SCALE + 1,
          wallHeight * RENDER_SCALE
        );
      }
    }
  ...
  ctx.fillText('═══ FIRST-PERSON MODE ═══', 10, 25);

  ctx.font = '14px monospace';
  ctx.fillStyle = '#0f0';
  const angle = Math.round((playerAngle * 180 / Math.PI) % 360);
  ctx.fillText(`HEADING: ${angle}°`, 10, 45);
  ctx.fillText(`POS: [${Math.floor(player.x)}, ${Math.floor(player.y)}]`, 10, 65);

  ctx.fillStyle = '#ffb300';
  ctx.fillText('[W/A/S/D] MOVE  [Q/E] TURN  [V] TOGGLE VIEW', 10, canvas.height - 15);
...
function drawCurrentLevel(time = 0) {
  ...
  // ========================================
  // TOP-DOWN MODE (ORIGINAL RENDERING)
  // ========================================
