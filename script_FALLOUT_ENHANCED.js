...
async function init() {
    ...
    // Init Enhanced Dust System (Mojave Atmosphere)
    for(let i=0; i<80; i++) { // More particles for desert feel
        dustMotes.push({
            ...
            alpha: Math.random() * 0.6 + 0.2, // More visible
            type: Math.random() > 0.7 ? 'sand' : 'dust' // Different particle types
        });
    });
    }
...
function drawSprite(ctx, type, x, y, size, time) {
    ...
    // 🏜️ MOJAVE DESERT COLOR GRADING (Warm wasteland atmosphere)
    if (config.mapType === 'cave') {
        ...
        if (currentLevelIndex >= 0) {
            // Surface caves: warm golden hour lighting
            warmGrad.addColorStop(0, 'rgba(255, 200, 120, 0.15)');
            warmGrad.addColorStop(0.6, 'rgba(255, 180, 100, 0.10)');
            warmGrad.addColorStop(1, 'rgba(200, 100, 60, 0.05)');
        } else {
            // Underground caves: cooler but still desert-toned
            warmGrad.addColorStop(0, 'rgba(200, 180, 150, 0.08)');
            warmGrad.addColorStop(1, 'rgba(150, 120, 100, 0.03)');
        }
...
function drawCRTEffects(ctx, width, height) {
    ...
    // 🏜️ MOJAVE HEAT SHIMMER (for surface caves)
    if (config.mapType === 'cave' && currentLevelIndex >= 0) {
        const heatGrad = ctx.createLinearGradient(0, 0, 0, height);
        heatGrad.addColorStop(0, "rgba(255, 200, 150, 0)");
        heatGrad.addColorStop(0.3, "rgba(255, 200, 150, 0.08)");
        heatGrad.addColorStop(1, "rgba(255, 180, 100, 0.15)");
...
function drawCurrentLevel(time = 0) {
    ...
    // --- ULTRA-ENHANCED FALLOUT MOJAVE RENDERING ---
    if (patternType === 'cave') {
        ...
        for (let x = 0; x < config.cols; x++) {
            for (let y = 0; y < config.rows; y++) {
                if (data.grid[x][y] >= 1) {
                    ...
                    // 🏜️ BASE LAYER: Organic jagged rock shape
                    ...
                    // 🪨 STRATIFIED ROCK LAYERS (Sedimentary bands)
                    if ((x + y) % 3 === 0) {
                        ctx.fillStyle = pal.floor.dark;
                        ctx.globalAlpha = 0.3;
                        const bandY = py + (getOffset(x, y, 9) * gs * 0.6);
                        ctx.fillRect(px, bandY, gs, 2 + getOffset(x, y, 10) * 2);
                        ctx.globalAlpha = 1.0;
                    }

                    // ☀️ SUN-BLEACHED HIGHLIGHTS (for surface caves)
                    if (currentLevelIndex >= 0 && (x + y) % 4 === 0) {
                        ctx.fillStyle = pal.floor.light;
                        ctx.globalAlpha = 0.2;
                        ctx.beginPath();
                        ctx.arc(px + gs/2, py + gs/2, gs * 0.3, 0, Math.PI * 2);
                        ctx.fill();
                        ctx.globalAlpha = 1.0;
                    }

                    // 🌵 SAND RIPPLE TEXTURE (wind patterns)
                    if (currentLevelIndex >= 0) {
                        ctx.strokeStyle = pal.floor.noise;
                        ctx.globalAlpha = 0.15;
                        ctx.lineWidth = 1;
                        for (let r = 0; r < 3; r++) {
                            const rippleY = py + (r * gs / 3) + Math.sin(x * 0.5) * 2;
                            ctx.beginPath();
                            ctx.moveTo(px, rippleY);
                            ctx.lineTo(px + gs, rippleY);
                            ctx.stroke();
                        }
                        ctx.globalAlpha = 1.0;
                    }

                    // 🦴 SCATTERED BONES & DEBRIS (Wasteland atmosphere)
                    if (getOffset(x, y, 11) > 0.92) {
                        ctx.fillStyle = '#e5e7eb';
                        ctx.globalAlpha = 0.6;
                        const boneX = px + getOffset(x, y, 12) * gs * 0.8;
                        const boneY = py + getOffset(x, y, 13) * gs * 0.8;
                        ctx.fillRect(boneX, boneY, 6, 2);
                        ctx.fillRect(boneX + 3, boneY - 2, 2, 4);
                        ctx.globalAlpha = 1.0;
                    }

                    // 🎨 TRIBAL CAVE PAINTINGS (Ancient markings)
                    if (currentLevelIndex < 0 && getOffset(x, y, 14) > 0.96) {
                    ...
                    // 💎 MINERAL VEINS (Quartz/ore deposits)
                    if (getOffset(x, y, 15) > 0.90) {
                        ctx.strokeStyle = pal.wall.highlight;
                        ctx.globalAlpha = 0.5;
                        ctx.lineWidth = 1;
                        ctx.beginPath();
                        ctx.moveTo(px + getOffset(x, y, 16) * gs, py);
                        ctx.lineTo(px + getOffset(x, y, 17) * gs, py + gs);
                        ctx.stroke();
                        ctx.globalAlpha = 1.0;
                    }

                    // 🌊 WATER POOLS (Enhanced radiation effect)
                    if (data.grid[x][y] === 2) {
                        ctx.fillStyle = 'rgba(59, 130, 246, 0.4)';
                        ctx.fill();

                        // Radioactive glow pulse
                        const radPulse = Math.sin(time / 300 + x + y) * 0.3 + 0.7;
                        ctx.fillStyle = `rgba(132, 204, 22, ${radPulse * 0.3})`;
                        ctx.beginPath();
                        ctx.arc(px + gs/2, py + gs/2, gs * 0.4, 0, Math.PI * 2);
                        ctx.fill();
    ...
    // --- ENHANCED LIGHTING ENGINE (Fallout Atmosphere) ---
    // Deeper shadows for caves, lighter for surface
    const shadowIntensity = (config.mapType === 'cave' && currentLevelIndex < 0) ? 0.65 : 0.4;
    ...
    // 🌪️ ENHANCED MOJAVE DUST PARTICLES
    for(let m of dustMotes) {
        ...
        if (dist < 200) {
            // Calculate particle visibility based on distance from light
            const visibility = 1 - (dist / 200);

            if (m.type === 'sand') {
                // Desert sand particles (warm amber)
                ctx.fillStyle = `rgba(255, 200, 150, ${visibility * 0.6})`;
                ctx.beginPath();
                ctx.arc(m.x, m.y, m.size, 0, Math.PI * 2);
                ctx.fill();
            } else {
                // Regular dust (cooler tone)
                ctx.fillStyle = `rgba(200, 220, 200, ${visibility * 0.4})`;
                ctx.fillRect(m.x, m.y, m.size, m.size);
            }

            // Add glow for particles very close to light
            if (dist < 80 && config.mapType === 'cave') {
                ctx.fillStyle = `rgba(255, 230, 180, ${(1 - dist/80) * 0.3})`;
                ctx.beginPath();
    ...
    if (data.decorations) for (let deco of data.decorations) {
        ...
        if (deco.type === 'fire_barrel' || deco.type === 'campfire') {
            const flicker = Math.sin(time / 80) * 8 + Math.random() * 5; // More dramatic flicker
            radius = gs * 3.5 + flicker; // Larger warm glow
            colorStart = 'rgba(255, 180, 80, 0.8)'; // Warmer, more intense	
            colorMid = 'rgba(234, 88, 12, 0.3)'; // Stronger orange mid-tone
        }	
        else if (deco.type === 'rad_puddle') {
            const pulse = Math.sin(time / 600);
            radius = gs * 3 + (pulse * 15); // Much larger radius
            colorStart = 'rgba(180, 255, 100, 0.7)'; // Brighter glow
            colorMid = 'rgba(132, 204, 22, 0.4)'; // More intense mid-tone
        }
