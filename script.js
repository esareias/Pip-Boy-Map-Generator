...
function drawCurrentLevel(time = 0) {
    ...
    // FOG RENDERING (Adaptive: Organic for Wasteland, Grid for Vault)
    for (let x = 0; x < config.cols; x++) {
        for (let y = 0; y < config.rows; y++) {
            if (config.fogEnabled && !isLocationRevealed(data, x, y)) {
                ...
                // ====================================
                // ADAPTIVE RENDERING BASED ON MAP TYPE
                // ====================================
                const isWasteland = (patternType === 'cave' || patternType === 'ruins');

                if (isWasteland) {
                    // ==========================================
                    // ORGANIC FOG (Cave/Ruins/Mojave Wasteland)
                    // ==========================================
                    ...
                    // LAYER 2: Organic fog shape (not perfect squares!)
                    // Use noise-based distortion for natural fog boundaries
                    const noiseScale = 0.15;
                    const distortX1 = Math.sin(x * noiseScale + time * 0.01) * 8;
                    const distortY1 = Math.cos(y * noiseScale + time * 0.008) * 8;
                    const distortX2 = Math.sin((x + y) * noiseScale * 0.7 - time * 0.012) * 6;
                    const distortY2 = Math.cos((x - y) * noiseScale * 0.8 + time * 0.015) * 6;

                    // Create irregular fog patch using path
                    ctx.save();
                    ctx.beginPath();

                    // Define 8 points around the tile with distortion
                    const points = [
                        {x: px + distortX1, y: py + distortY1},
                        {x: px + gs/2 + distortX2, y: py + distortY2},
                        {x: px + gs + distortX1, y: py + distortY1},
                        {x: px + gs + distortX2, y: py + gs/2 + distortY2},
                        {x: px + gs + distortX1, y: py + gs + distortY1},
                        {x: px + gs/2 + distortX2, y: py + gs + distortY2},
                        {x: px + distortX1, y: py + gs + distortY1},
                        {x: px + distortX2, y: py + gs/2 + distortY2}
                    ];

                    ctx.moveTo(points[0].x, points[0].y);
                    for (let i = 1; i < points.length; i++) {
                        // Use curves for smoother organic shapes
                        const p1 = points[i];
                        const p2 = points[(i + 1) % points.length];
                        ctx.quadraticCurveTo(p1.x, p1.y, (p1.x + p2.x)/2, (p1.y + p2.y)/2);
                    }
                    ctx.closePath();
                    ctx.clip(); // Clip to organic shape

                    // LAYER 3: Multi-pass volumetric fog (within organic clip)
                    // Pass 1: Large rolling banks
                    const fog1X = (px + time * 0.018) % 512;
                    const fog1Y = (py + time * 0.010) % 512;
                    ctx.globalAlpha = 0.65;
                    ctx.drawImage(cloudCanvas, fog1X, fog1Y, gs * 1.8, gs * 1.8, px - gs/3, py - gs/3, gs * 1.8, gs * 1.8);

                    // Pass 2: Medium tendrils
                    const fog2X = (px - time * 0.028 + Math.sin(y * 0.3) * 30) % 512;
                    const fog2Y = (py + time * 0.015 + Math.cos(x * 0.3) * 30) % 512;
                    ctx.globalAlpha = 0.45;
                    ctx.drawImage(cloudCanvas, fog2X, fog2Y, gs * 1.2, gs * 1.2, px - gs/6, py - gs/6, gs * 1.4, gs * 1.4);

                    // Pass 3: Fast wisps
                    const fog3X = (px + time * 0.045 + Math.sin(time/200 + x) * 25) % 512;
                    const fog3Y = (py + time * 0.025 + Math.cos(time/250 + y) * 25) % 512;
                    ctx.globalAlpha = 0.35;
                    ctx.drawImage(cloudCanvas, fog3X, fog3Y, gs, gs, px, py, gs * 1.1, gs * 1.1);

                    ctx.globalAlpha = 1.0;
                    ctx.restore(); // Remove clip

                    // LAYER 4: Dust particles (Mojave feel)
                    if ((x + y + Math.floor(time / 80)) % 7 === 0) {
                        const dustX = px + Math.sin(time / 150 + x * 0.5) * 10;
                        const dustY = py + Math.cos(time / 180 + y * 0.5) * 10;
                        const dustSize = gs * (0.6 + Math.sin(time / 120 + x + y) * 0.3);

                        const dustGrad = ctx.createRadialGradient(
                            dustX + gs/2, dustY + gs/2, 0,
                            dustX + gs/2, dustY + gs/2, dustSize/2
                        );
                        dustGrad.addColorStop(0, 'rgba(120, 100, 80, 0.18)');
                    ...
                    // LAYER 5: Atmospheric haze (distance fade)
                    const centerX = config.cols / 2;
                    const centerY = config.rows / 2;
                    const distFromCenter = Math.sqrt((x - centerX) ** 2 + (y - centerY) ** 2);
                    const maxDist = Math.sqrt(centerX ** 2 + centerY ** 2);
                    const hazeFade = Math.min(distFromCenter / maxDist, 1);

                    ctx.fillStyle = `rgba(10, 10, 8, ${hazeFade * 0.35})`;
                    ctx.fillRect(px, py, gs, gs);

                    // LAYER 6: Soft edge transition to revealed areas
                    let edgeRevealCount = 0;
                    const neighbors = [
                        {dx: -1, dy: 0}, {dx: 1, dy: 0},
                        {dx: 0, dy: -1}, {dx: 0, dy: 1},
                        {dx: -1, dy: -1}, {dx: 1, dy: -1},
                    ...
                    if (edgeRevealCount > 0) {
                        const edgeFade = edgeRevealCount / 8;
                        const edgeGrad = ctx.createRadialGradient(
                            px + gs/2, py + gs/2, 0,
                            px + gs/2, py + gs/2, gs * 1.5
                        );
                        edgeGrad.addColorStop(0, `rgba(10, 10, 8, ${edgeFade * 0.5})`);
                    ...
                    // ==========================================
                    // STRUCTURED FOG (Vault - Perfect Squares)
                    // ==========================================
