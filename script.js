...
function drawCurrentLevel(time = 0) {
            ...
            for (let y = 0; y < config.rows; y++) {
                if (data.grid[x][y] >= 1) {
                    ...
                    if (data.grid[x][y] === 2) {
                        ...
                        // ☢️ Subtle radioactive glow
                        const glowPulse = Math.sin(time / 400 + x + y) * 0.2 + 0.3;
                        ctx.fillStyle = `rgba(132, 204, 22, ${glowPulse})`;
                        ctx.globalAlpha = 0.5;
                        ctx.beginPath();
            ...
            for (let y = 0; y < config.rows; y++) {
                if (data.grid[x][y] >= 1) {	
                    ...
                    if (data.grid[x][y] === 2) {
                        ...
                        // 🪨 Subtle rock stratification (cave only)
                        if (config.mapType === 'cave' && (x + y) % 4 === 0) {
                            ctx.fillStyle = pal.floor.dark;
                            ctx.globalAlpha = 0.2;
                            ctx.fillRect(px, py + gs * 0.4, gs, 2);
                            ctx.globalAlpha = 1.0;
                        }
    ...
    if (data.decorations) for (let deco of data.decorations) {
        ...
        if (deco.type === 'fire_barrel' || deco.type === 'campfire') {
            const flicker = Math.sin(time / 80) * 8 + Math.random() * 5;
    ...
    ctx.globalCompositeOperation = 'source-over';
    // 🏜️ Warm desert atmosphere overlay
    if (config.mapType === 'cave') {
        ...
        warmOverlay.addColorStop(0, 'rgba(255, 200, 120, 0.12)');
        warmOverlay.addColorStop(1, 'rgba(200, 150, 100, 0.05)');
