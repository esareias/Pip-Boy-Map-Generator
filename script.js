...
let fov = Math.PI / 3;
let rayCount = 320; // Higher resolution for texture sampling
...
// Texture canvases for first-person sampling
let fpWallTexture = null;
let fpFloorTexture = null;
...
// --- SAFE PATCH: Institutional and sanitation room relations additions ---
try {
  if (typeof ROOM_RELATIONS === 'object') {
    ROOM_RELATIONS["Classroom"] = { tags: ["Clean", "Learning"], link: ["Hallway", "Library Archives"], avoid: ["Industrial", "Hazard", "Bedroom"] };
    ROOM_RELATIONS["Gymnasium"] = { tags: ["Large", "Rec"], link: ["Locker Room", "Cafeteria"], avoid: ["Office", "Quiet"] };
    ROOM_RELATIONS["Library Archives"] = { tags: ["Quiet", "Paper"], link: ["Study Hall", "Admin Office"], avoid: ["Water", "Fire", "Loud"] };
    ROOM_RELATIONS["Principal's Office"] = { tags: ["Office", "Admin"], link: ["Admin Office", "Hallway"], avoid: ["Classroom"] };
    ROOM_RELATIONS["Main Hall"] = { tags: ["Hub", "Clean"], link: ["Classroom", "Admin Office", "Gymnasium"], avoid: ["Dirty"] };
    ROOM_RELATIONS["Auditorium"] = { tags: ["Large", "Social"], link: ["Main Hall", "Hallway"], avoid: ["Bedroom", "Industrial"] };
    ROOM_RELATIONS["Faculty Lounge"] = { tags: ["Office", "Clean"], link: ["Classroom", "Admin Office"], avoid: ["Student"] };
    // Sanitation and kitchen separation
    ROOM_RELATIONS["Kitchen"] = { tags: ["Service", "Loud"], link: ["Cafeteria", "Dining Hall", "Cold Storage", "Pantry"], avoid: ["Bedroom", "Toilet", "Morgue", "Office", "Restroom"] };
    ROOM_RELATIONS["Cafeteria"] = { tags: ["Food", "Social"], link: ["Kitchen", "Hallway"], avoid: ["Restroom", "Morgue", "Toxic Pit", "Toilet"] };
    ROOM_RELATIONS["Restroom"] = { tags: ["Water", "Private"], link: ["Hallway"], avoid: ["Kitchen", "Cafeteria", "Chapel", "Food"] };
    ROOM_RELATIONS["Boiler Room"] = { tags: ["Industrial", "Dirty"], link: ["Janitor Closet", "Basement"], avoid: ["Nursery", "Executive Suite", "Sterile", "Clean"] };
  }
} catch (e) { /* no-op */ }
const ROOM_LOGIC = {
    // CAVE / MOJAVE
...
// ========================================
// FIRST-PERSON MODE - KEYBOARD CONTROLS
// ========================================
window.addEventListener('keydown', (e) => {
  ...
  if (key === 'q' || key === 'arrowleft') turnLeft = true;
  if (key === 'e' || key === 'arrowright') turnRight = true;
});
...
window.addEventListener('keyup', (e) => {
  ...
  if (key === 'q' || key === 'arrowleft') turnLeft = false;
  if (key === 'e' || key === 'arrowright') turnRight = false;
});
...
// ========================================
// FIRST-PERSON MODE - CORE FUNCTIONS
// ========================================

function toggleFirstPersonMode() {
  firstPersonMode = !firstPersonMode;
  if (firstPersonMode) {
    log('FIRST-PERSON MODE ACTIVATED', 'var(--pip-green)');
    log('W/A/S/D=MOVE, Q/E=TURN, V=TOGGLE VIEW', 'var(--pip-amber)');
    const player = tokens.find(t => t.name !== 'OVERSEER_TOKEN_ID') || tokens[0];
    if (!player) log('WARNING: NO PLAYER TOKEN DETECTED', 'var(--pip-amber)');

    // Generate textures from current map type
    generateFirstPersonTextures();
  } else {
    log('TOP-DOWN MODE ACTIVATED', 'var(--pip-green)');
  }
  drawCurrentLevel();
}

function generateFirstPersonTextures() {
  // Get current palette
  ...
  if (viewMode === 'sector') {
    ...
    if (config.mapType === 'ruins') {
      pal = PALETTES.interiorruins;
      patternType = 'interiorruins';
    } else if (config.mapType === 'cave') {
      pal = PALETTES.interiorcave;
      patternType = 'interiorcave';
    }
  }

  // Create wall texture using the same pattern system as top-down
  ...
  wCtx.fillStyle = '#' + pal.wall.front;
  wCtx.fillRect(0, 0, 256, 256);

  // Add the same pixel pattern used in top-down view
  const wallPattern = createPixelPattern(wCtx, patternType, 'wall');
  if (wallPattern) {
    wCtx.fillStyle = wallPattern;
    wCtx.fillRect(0, 0, 256, 256);
  }
  ...
  for (let i = 0; i < 100; i++) {
    ...
    wCtx.fillStyle = `rgba(${brightness > 0 ? 255 : 0}, ${brightness > 0 ? 255 : 0}, ${brightness > 0 ? 255 : 0}, ${Math.abs(brightness) / 100})`;
    wCtx.fillRect(x, y, 2, 2);
  }

  fpWallTexture = wallCanvas;
  ...
  fCtx.fillStyle = '#' + pal.floor.base;
  fCtx.fillRect(0, 0, 256, 256);

  // Add the same pixel pattern used in top-down view
  const floorPattern = createPixelPattern(fCtx, patternType, 'floor');
  if (floorPattern) {
    fCtx.fillStyle = floorPattern;
    fCtx.fillRect(0, 0, 256, 256);
  }

  fpFloorTexture = floorCanvas;
}
...
function updateFirstPersonMovement() {
  ...
  // Rotation
  if (turnLeft) playerAngle -= rotationSpeed;
  if (turnRight) playerAngle += rotationSpeed;
  ...
  if (moveX !== 0 || moveY !== 0) {
    ...
    if (gridX >= 0 && gridX < data.grid.length && gridY >= 0 && gridY < data.grid[0].length) {
      if (data.grid[gridX][gridY] === 1) {
        player.x = newX;
        player.y = newY;
        if (isHost && conn) syncData();
      }
    }
  }
}

function castRay(data, startX, startY, angle) {
  const grid = data.grid;
  if (!grid) return { distance: 1000, hit: false, hitX: 0, hitY: 0 };

  let x = startX / config.gridSize;
  let y = startY / config.gridSize;
  const stepSize = 0.05;
  ...
  while (distance < maxDistance) {
    ...
    if (gridX < 0 || gridX >= grid.length || gridY < 0 || gridY >= grid[0].length) {
      return { distance: distance, hit: true, hitX: x * config.gridSize, hitY: y * config.gridSize };
    }
  ...
  return { distance: maxDistance, hit: false, hitX: 0, hitY: 0 };
}
...
function drawFirstPerson() {
  ...
  if (!data || !data.grid) {
    ...
    ctx.fillText('NO MAP DATA - PRESS [SCAN LEVEL] FIRST', canvas.width/2 - 200, canvas.height/2);
    return;
  }
  ...
  if (!player) {
    ...
    ctx.fillStyle = '#f00';
    ctx.font = 'bold 20px monospace';
    ctx.fillText('NO PLAYER TOKEN - DEPLOY A UNIT FIRST', canvas.width/2 - 220, canvas.height/2);
    return;
  }

  // Regenerate textures if they don't exist
  if (!fpWallTexture || !fpFloorTexture) {
    generateFirstPersonTextures();
  }
  ...
  let pal = PALETTES.vault;
  if (viewMode === 'sector') {
    if (config.mapType === 'ruins') pal = PALETTES.ruins;
    else if (config.mapType === 'cave') pal = PALETTES.cave;
  } else {
    if (config.mapType === 'ruins') pal = PALETTES.interiorruins;
    else if (config.mapType === 'cave') pal = PALETTES.interiorcave;
  }
  ...
  // Draw ceiling using background color
  ...
  if (fpFloorTexture) {
    ...
    for (let y = Math.floor(halfHeight); y < screenHeight; y++) {
      const distance = (screenHeight / (2 * y - screenHeight + 1)) * 3;
      const fogFactor = Math.max(0, Math.min(1, 1 - distance / 30));

      // Calculate floor position at this row
      const floorX = player.x + Math.cos(playerAngle) * distance * config.gridSize;
      const floorY = player.y + Math.sin(playerAngle) * distance * config.gridSize;

      // Sample floor texture
      const texX = Math.floor(floorX) % 256;
      const texY = Math.floor(floorY) % 256;

      // Get pixel from floor texture
      const imgData = fpFloorTexture.getContext('2d').getImageData(texX, texY, 1, 1).data;
      const r = Math.floor(imgData[0] * fogFactor);
      const g = Math.floor(imgData[1] * fogFactor);
      const b = Math.floor(imgData[2] * fogFactor);

      ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
      ctx.fillRect(0, y * RENDER_SCALE, canvas.width, RENDER_SCALE);
    }
  }

  // Cast rays for walls with texture sampling
  const stripWidth = screenWidth / rayCount;

  for (let i = 0; i < rayCount; i++) {
    const rayAngle = (playerAngle - fov / 2) + (fov * i / rayCount);
    const hit = castRay(data, player.x, player.y, rayAngle);

    if (hit.hit && fpWallTexture) {
      // Fix fish-eye distortion
      const correctedDistance = hit.distance * Math.cos(rayAngle - playerAngle);
      const wallHeight = (screenHeight / correctedDistance) * 8;
      const wallTop = halfHeight - wallHeight / 2;

      // Calculate texture coordinate
      const texX = Math.floor((hit.hitX + hit.hitY) * 10) % 256;

      // Draw textured wall strip
      for (let y = Math.max(0, Math.floor(wallTop)); y < Math.min(screenHeight, Math.ceil(wallTop + wallHeight)); y++) {
        // Calculate texture Y coordinate based on wall height position
        const texY = Math.floor(((y - wallTop) / wallHeight) * 256) % 256;

        // Sample texture
        const imgData = fpWallTexture.getContext('2d').getImageData(texX, texY, 1, 1).data;

        // Apply distance-based fog
        const fogFactor = Math.max(0, Math.min(1, 1 - correctedDistance / 30));
        const r = Math.floor(imgData[0] * fogFactor);
        const g = Math.floor(imgData[1] * fogFactor);
        const b = Math.floor(imgData[2] * fogFactor);
        ...
        ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
        ctx.fillRect(
          i * stripWidth * RENDER_SCALE,
          y * RENDER_SCALE,
          Math.ceil(stripWidth * RENDER_SCALE) + 1,
          RENDER_SCALE
        );
      }
    }
  ...
  ctx.fillText('═══ FIRST-PERSON MODE ═══', 15, 30);

  ctx.font = '15px monospace';
  const angle = Math.round((playerAngle * 180 / Math.PI) % 360);
  ctx.fillText(`HEADING: ${angle}°`, 15, 55);
  ctx.fillText(`POS: [${Math.floor(player.x)}, ${Math.floor(player.y)}]`, 15, 75);

  ctx.fillStyle = '#ffb300';
  ctx.font = '14px monospace';
  ctx.fillText('[W/A/S/D] MOVE  [Q/E] TURN  [V] TOGGLE VIEW', 15, canvas.height - 15);
...
canvas.height = config.height * RENDER_SCALE;
    ...
    // Base: Deep darkness
    cCtx.fillStyle = 'rgba(5, 10, 5, 0.9)';
    cCtx.fillRect(0, 0, 512, 512);

    // ====================================
    // LAYER 1: Large fog banks (low frequency)
    // ====================================
    for (let i = 0; i < 50; i++) {
        const x = Math.random() * 512;
        const y = Math.random() * 512;
        const r = Math.random() * 120 + 80; // Larger clouds

        const g = cCtx.createRadialGradient(x, y, 0, x, y, r);
        const opacity = Math.random() * 0.3 + 0.2;
        g.addColorStop(0, `rgba(40, 60, 45, ${opacity})`);
        g.addColorStop(0.5, `rgba(25, 40, 30, ${opacity * 0.6})`);
        ...
        // Seamless tiling (wrap edges)
        if (x < r) {
            cCtx.fillStyle = g;
            cCtx.beginPath();
            cCtx.arc(x + 512, y, r, 0, Math.PI * 2);
            cCtx.fill();
        }
        if (x > 512 - r) {
            cCtx.fillStyle = g;
        ...
        if (y < r) {
            cCtx.fillStyle = g;
            cCtx.beginPath();
            cCtx.arc(x, y + 512, r, 0, Math.PI * 2);
            cCtx.fill();
        }
        if (y > 512 - r) {
            cCtx.fillStyle = g;
    ...
    for (let i = 0; i < 200; i++) {
        ...
        g.addColorStop(0, `rgba(60, 80, 65, ${opacity})`);
        g.addColorStop(1, 'rgba(0, 0, 0, 0)');
    ...
    for (let i = 0; i < 800; i++) {
        ...
        g.addColorStop(0, `rgba(50, 70, 55, ${opacity})`);
        g.addColorStop(1, 'rgba(0, 0, 0, 0)');
    ...
    // ====================================
    // LAYER 4: Radioactive wisps (Fallout touch)
    // ====================================
    ...
    for (let i = 0; i < 30; i++) {
        ...
        const g = cCtx.createRadialGradient(x, y, 0, x, y, r);
        g.addColorStop(0, 'rgba(80, 200, 120, 0.08)');
        g.addColorStop(0.6, 'rgba(60, 150, 90, 0.04)');
        g.addColorStop(1, 'rgba(0, 0, 0, 0)');
    ...
    // ====================================
    // LAYER 5: Density variation (dark spots)
    // ====================================
    for (let i = 0; i < 40; i++) {
        const x = Math.random() * 512;
        const y = Math.random() * 512;
...
function generateInterior(key, name) {
    ...
    if (config.mapType === 'ruins' && currentLevelIndex < 0) {
        // SAFE PATCH: Context-aware basement override
        try {
          const __parentArch = getArchetype(name || (interiorData && interiorData[key] && interiorData[key].name) || '');
          if (__parentArch === 'INSTITUTIONAL' || __parentArch === 'COMMERCIAL') {
            if (Math.random() < 0.7) {
              effectiveArchKey = 'GENERIC';
            } else {
              effectiveArchKey = (Math.random() < 0.5) ? 'SEWER' : 'CULT';
            }
          }
        } catch (e) { /* no-op */ }
  ...
  for(let i = -2; i <= 2; i++) {
    if(floorData[i]) {
      ...
      if(floorData[i].labels && floorData[i].labels.length > 0) {
        ...
        const nonStairsLabels = floorData[i].labels.filter(lbl => 
          !lbl.text.includes('STAIRS') && 
          !lbl.text.includes('Street') && 
          !lbl.text.includes('Road') &&
          !lbl.text.includes('Alley')
        );
        
        if (nonStairsLabels.length > 0) {
          report += `\nENTERABLE LOCATIONS:\n`;
          nonStairsLabels.forEach(lbl => {
      ...
      // Loot containers
      if(floorData[i].loot && floorData[i].loot.length > 0) {
        report += `LOOT CONTAINERS:\n`;
...
function generateRuins(data, density, anchors) {
    ...
    if (currentLevelIndex < 0) {
        ...
        const allowedRooms = SUB_THEMES[selectedThemeKey];
            ...
            // SAFE PATCH: Smart theme remap based on archetype
            try {
              if (anchors && anchors.length > 0 && anchors[0].upperName) {
                const __arch = getArchetype(anchors[0].upperName);
                if (__arch === 'INSTITUTIONAL') {
                  if (!SUB_THEMES['institutional']) SUB_THEMES['institutional'] = ["Classroom", "Library Row", "Cafeteria", "Boiler Room", "Study Hall", "Office"];
                  selectedThemeKey = 'institutional';
                  foundSmartTheme = true;
                } else if (__arch === 'MEDICAL' || __arch === 'POLICE' || __arch === 'CULT') {
                  selectedThemeKey = 'creepier'; foundSmartTheme = true;
                } else if (__arch === 'INDUSTRIAL' || __arch === 'BUNKER' || __arch === 'SEWER') {
                  selectedThemeKey = 'industrial'; foundSmartTheme = true;
                } else if (__arch === 'COMMERCIAL' || __arch === 'RETAIL' || __arch === 'ENTERTAINMENT' || __arch === 'RESIDENTIAL') {
                  selectedThemeKey = 'residential'; foundSmartTheme = true;
                }
              }
            } catch (e) { /* no-op */ }
...
ctx.scale(RENDER_SCALE * zoomLevel, RENDER_SCALE * zoomLevel);
    ...
    // FOG RENDERING (Volumetric Multi-Layer System)
    for (let x = 0; x < config.cols; x++) {
        for (let y = 0; y < config.rows; y++) {
            if (config.fogEnabled && !isLocationRevealed(data, x, y)) {
                ...
                // ====================================
                // LAYER 1: Deep Darkness Base
                // ====================================
                ctx.fillStyle = '#0a0f0a'; // Dark green-black
                ctx.fillRect(px, py, gs, gs);

                // ====================================
                // LAYER 2: Volumetric Fog (3 Passes)
                // ====================================

                // Pass 1: Large slow-moving fog banks
                const fog1X = (px + time * 0.015) % 512;
                const fog1Y = (py + time * 0.008) % 512;
                ctx.globalAlpha = 0.6;
                ctx.drawImage(cloudCanvas, fog1X, fog1Y, gs * 1.5, gs * 1.5, px - gs/4, py - gs/4, gs * 1.5, gs * 1.5);

                // Pass 2: Medium swirling fog
                const fog2X = (px - time * 0.025) % 512;
                const fog2Y = (py + time * 0.012) % 512;
                ctx.globalAlpha = 0.4;
                ctx.drawImage(cloudCanvas, fog2X, fog2Y, gs, gs, px, py, gs, gs);

                // Pass 3: Fast wispy details
                const fog3X = (px + time * 0.04 + Math.sin(time/300 + x) * 20) % 512;
                const fog3Y = (py + time * 0.02 + Math.cos(time/400 + y) * 20) % 512;
                ctx.globalAlpha = 0.3;
                ctx.drawImage(cloudCanvas, fog3X, fog3Y, gs * 0.8, gs * 0.8, px, py, gs, gs);

                ctx.globalAlpha = 1.0;

                // ====================================
                // LAYER 3: Radioactive Tint (Fallout vibe)
                // ====================================
                const radGlow = Math.sin(time / 800 + x * 0.3 + y * 0.3) * 0.5 + 0.5;
                ctx.fillStyle = `rgba(80, 200, 120, ${0.08 + radGlow * 0.06})`;
                ctx.fillRect(px, py, gs, gs);

                // ====================================
                // LAYER 4: Depth Gradient (Closer = Darker)
                // ====================================
                const centerX = config.cols / 2;
                const centerY = config.rows / 2;
                const distFromCenter = Math.sqrt((x - centerX) ** 2 + (y - centerY) ** 2);
                const maxDist = Math.sqrt(centerX ** 2 + centerY ** 2);
                const depthFade = Math.min(distFromCenter / maxDist, 1);

                ctx.fillStyle = `rgba(0, 0, 0, ${depthFade * 0.4})`;
                ctx.fillRect(px, py, gs, gs);

                // ====================================
                // LAYER 5: Animated Fog Wisps
                // ====================================
                if ((x + y + Math.floor(time / 100)) % 8 === 0) {
                    const wispX = px + Math.sin(time / 200 + x) * 8;
                    const wispY = py + Math.cos(time / 300 + y) * 8;
                    const wispSize = gs * (0.8 + Math.sin(time / 150 + x + y) * 0.2);

                    const wispGrad = ctx.createRadialGradient(
                        wispX + gs/2, wispY + gs/2, 0,
                        wispX + gs/2, wispY + gs/2, wispSize/2
                    );
                    wispGrad.addColorStop(0, 'rgba(100, 150, 120, 0.15)');
                    wispGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');

                    ctx.fillStyle = wispGrad;
                    ctx.fillRect(wispX, wispY, wispSize, wispSize);
                }

                // ====================================
                // LAYER 6: Edge Vignette (Fade to revealed areas)
                // ====================================
                // Check neighboring tiles and create soft edges
                let edgeFade = 0;
                const neighbors = [
                    {dx: -1, dy: 0}, {dx: 1, dy: 0},
                    {dx: 0, dy: -1}, {dx: 0, dy: 1},
                ...
                for (let n of neighbors) {
                    ...
                    if (nx >= 0 && nx < config.cols && ny >= 0 && ny < config.rows) {
                        if (isLocationRevealed(data, nx, ny)) {
                            edgeFade += 0.125; // Each revealed neighbor adds fade
                        }
                    }
                }

                if (edgeFade > 0) {
                    // Create gradient toward revealed areas
                    const edgeGrad = ctx.createRadialGradient(
                        px + gs/2, py + gs/2, 0,
                        px + gs/2, py + gs/2, gs
                    );
                    edgeGrad.addColorStop(0, `rgba(10, 15, 10, ${edgeFade * 0.6})`);
                    edgeGrad.addColorStop(1, 'rgba(10, 15, 10, 0)');
