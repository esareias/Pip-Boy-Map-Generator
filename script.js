...
let fov = Math.PI / 3;
let rayCount = 160; // Optimized ray count
...
// Cached wall/floor colors for fast rendering
let fpWallColors = null;
let fpFloorColors = null;
...
function toggleFirstPersonMode() {
  ...
  if (firstPersonMode) {
    ...
    if (!player) log('WARNING: NO PLAYER TOKEN', 'var(--pip-amber)');

    // Cache colors for fast rendering
    cacheFirstPersonColors();
  } else {
...
function cacheFirstPersonColors() {
  // Get current palette
  ...
  // Create array of wall colors with variation for texture effect
  fpWallColors = [];
  const baseWall = hexToRgb(pal.wall.front);
  const topWall = hexToRgb(pal.wall.top);

  for (let i = 0; i < 256; i++) {
    // Mix between front and top for variation
    const mix = (i % 16) / 16;
    const r = Math.floor(baseWall.r * (1 - mix) + topWall.r * mix);
    const g = Math.floor(baseWall.g * (1 - mix) + topWall.g * mix);
    const b = Math.floor(baseWall.b * (1 - mix) + topWall.b * mix);
  ...
  // Create array of floor colors with variation
  fpFloorColors = [];
  const baseFloor = hexToRgb(pal.floor.base);
  const darkFloor = hexToRgb(pal.floor.dark);

  for (let i = 0; i < 256; i++) {
    const mix = (i % 8) / 8;
    const r = Math.floor(baseFloor.r * (1 - mix) + darkFloor.r * mix);
    const g = Math.floor(baseFloor.g * (1 - mix) + darkFloor.g * mix);
    const b = Math.floor(baseFloor.b * (1 - mix) + darkFloor.b * mix);
...
function castRay(data, startX, startY, angle) {
  ...
  if (!grid) return { distance: 1000, hit: false };

  let x = startX / config.gridSize;
  let y = startY / config.gridSize;
  const stepSize = 0.1; // Larger step for performance
  ...
  while (distance < maxDistance) {
    ...
    if (gridX < 0 || gridX >= grid.length || gridY < 0 || gridY >= grid[0].length) {
      return { distance: distance, hit: true, texOffset: (x + y) * 10 };
    }
...
function drawFirstPerson() {
  ...
  const floorBase = hexToRgb(pal.floor.base);
  const floorDark = hexToRgb(pal.floor.dark);
  floorGradient.addColorStop(0, `rgb(${floorBase.r}, ${floorBase.g}, ${floorBase.b})`);
  floorGradient.addColorStop(1, `rgb(${floorDark.r}, ${floorDark.g}, ${floorDark.b})`);
  ctx.fillStyle = floorGradient;
  ctx.fillRect(0, canvas.height / 2, canvas.width, canvas.height / 2);

  // Add floor texture noise (single pass for performance)
  for (let i = 0; i < 500; i++) {
  ...
  for (let i = 0; i < rayCount; i++) {
    ...
    if (hit.hit) {
      ...
      // Get texture color with variation
      const texIndex = Math.floor(Math.abs(hit.texOffset)) % 256;
      const baseColor = fpWallColors[texIndex];

      // Apply distance fog
      const fogFactor = Math.max(0.1, Math.min(1, 1 - correctedDistance / 30));
      const r = Math.floor(baseColor.r * fogFactor);
      const g = Math.floor(baseColor.g * fogFactor);
      const b = Math.floor(baseColor.b * fogFactor);

      // Draw wall strip
      ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
      ctx.fillRect(
        i * stripWidth * RENDER_SCALE,
        wallTop * RENDER_SCALE,
        Math.ceil(stripWidth * RENDER_SCALE) + 1,
        wallHeight * RENDER_SCALE
      );

      // Add vertical detail lines for texture
      if (texIndex % 4 === 0) {
