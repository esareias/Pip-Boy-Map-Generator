...
let isoScale = 1.0;
let isoRotation = 0; // Can rotate the iso view
...
function toggleIsometricMode() {
  ...
  if (isometricMode) {
    ...
    log('2.5D TACTICAL VIEW ENABLED', 'var(--pip-amber)');
  } else {
...
function toIso(x, y) {
  return {
    x: (x - y) * 0.866, // cos(30°) for isometric angle
    y: (x + y) * 0.5    // sin(30°) for isometric angle
  };
}
...
function drawIsometric() {
  ...
  // Center the view
  const offsetX = config.width / 2 + mapOffsetX;
  const offsetY = config.height / 4 + mapOffsetY;
  const tileSize = config.gridSize * zoomLevel;

  // Draw grid in isometric perspective
  if (data.grid) {
    for (let x = 0; x < data.grid.length; x++) {
      for (let y = 0; y < data.grid[0].length; y++) {
        const worldX = x * tileSize;
        const worldY = y * tileSize;
        const iso = toIso(worldX, worldY);
        const screenX = (iso.x + offsetX) * RENDER_SCALE;
        const screenY = (iso.y + offsetY) * RENDER_SCALE;

        const tileW = tileSize * 0.866 * RENDER_SCALE;
        const tileH = tileSize * 0.5 * RENDER_SCALE;

        if (data.grid[x][y] === 1) {
          // Draw floor tile as isometric diamond
          ...
          // Draw wall with height
          const wallHeight = tileSize * 0.4 * RENDER_SCALE;
  ...
  // Draw tokens in isometric
  for (let t of tokens) {
    const iso = toIso(t.x * zoomLevel, t.y * zoomLevel);
    const screenX = (iso.x + offsetX) * RENDER_SCALE;
    const screenY = (iso.y + offsetY) * RENDER_SCALE;
    ...
    ctx.beginPath();
    ctx.ellipse(screenX, screenY + 10 * RENDER_SCALE, 8 * RENDER_SCALE, 4 * RENDER_SCALE, 0, 0, Math.PI * 2);
    ctx.fill();

    // Draw token
    if (t.img && t.img.complete) {
      const size = 24 * RENDER_SCALE;
      ctx.drawImage(t.img, screenX - size/2, screenY - size - 10 * RENDER_SCALE, size, size);
    } else {
      ctx.fillStyle = '#' + t.color;
      ctx.beginPath();
      ctx.arc(screenX, screenY - 10 * RENDER_SCALE, 10 * RENDER_SCALE, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    if (config.showLabels) {
      ...
      ctx.fillText(t.label, screenX, screenY - 35 * RENDER_SCALE);
    }
  }

  // Draw labels in isometric
  if (data.labels && config.showLabels) {
    for (let lbl of data.labels) {
      if (!lbl.visible) continue;
      const iso = toIso(lbl.x * zoomLevel, lbl.y * zoomLevel);
      const screenX = (iso.x + offsetX) * RENDER_SCALE;
      const screenY = (iso.y + offsetY) * RENDER_SCALE;
  ...
  ctx.textAlign = 'left';
  ctx.fillText('═══ ISOMETRIC VIEW ═══', 15, 25);

  ctx.font = '14px monospace';
  ctx.fillStyle = '#ffb300';
  ctx.fillText('[V] TOGGLE VIEW  [ZOOM] SCALE', 15, 50);
}
