...
function filterItemsByLootFocus(pool, focus) {
    ...
    if (focus === "HIGH_VALUE") {
        return pool.filter(i => i.v >= 70 || i.n.includes("Mini-Nuke") || i.n.includes("Stealth Boy") || i.n.includes("Pip-Boy") || i.n.includes("Plasma") || i.n.includes("Mesmetron"));
    }
    if (focus === "JUMPSUIT_GUNS") {
        return pool.filter(i => i.n.includes("Jumpsuit") || i.n.includes("Pistol") || i.n.includes("Armor") || i.n.includes("SMG") || i.n.includes("Rifle") || i.n.includes("Shotgun") || i.n.includes("Baton"));
    }
    ...
    if (focus === "PAPER_JUNK") {
        return pool.filter(i => i.n.includes("Money") || i.n.includes("Book") || i.n.includes("Pin") || i.n.includes("Teddy") || i.n.includes("Toy"));
    }
    if (focus === "MEDS") {
        return pool.filter(i => i.n.includes("Stimpak") || i.n.includes("Rad-X") || i.n.includes("RadAway") || i.n.includes("Doctor's Bag") || i.n.includes("Antivenom") || i.n.includes("Healing Powder") || i.n.includes("Poultice") || i.n.includes("Biogel") || i.n.includes("Fixer") || i.n.includes("Mentats"));
    }
    if (focus === "LOW_CAPS") {
        return pool.filter(i => i.n.includes("Money") || i.v < 10);
    }
    if (focus === "NUKA_COLA") {
        return pool.filter(i => i.n.includes("Nuka") || i.n.includes("Quantum") || i.n.includes("Rum") || i.n.includes("Beer") || i.n.includes("Buck Wild") || i.n.includes("Rot Gut") || i.n.includes("Jet") || i.n.includes("Psycho") || i.n.includes("Buffout"));
    }
    if (focus === "AMMO_EXP") {
        return pool.filter(i => i.n.includes("Dynamite") || i.n.includes("Molotov") || i.n.includes("Grenade") || i.n.includes("Mine") || i.n.includes("Pistol") || i.n.includes("Rifle") || i.n.includes("Shotgun") || i.n.includes("SMG") || i.n.includes("Mini-Nuke") || i.n.includes("Missile"));
    }
    if (focus === "FOOD_WATER" || focus === "SURVIVAL") {
        return pool.filter(i => i.n.includes("Water") || i.n.includes("Meat") || i.n.includes("Beans") || i.n.includes("Cram") || i.n.includes("Sugar Bombs") || i.n.includes("Fungus") || i.n.includes("Root") || i.n.includes("Flower") || i.n.includes("Gland"));
    }
...
function pickWeightedItem(type, requiredFocus = null, isLockedContainer = false) {
    ...
    // 1% Chance to upgrade a book to a Skill Book
    if (item.n === "Pre-War Book" && Math.random() < 0.01) {
        const books = ["Big Book of Science", "Dean's Electronics", "Duck and Cover!", "Grognak the Barbarian", "Guns and Bullets", "Tumblers Today"];
        item.n = books[Math.floor(Math.random() * books.length)];
        item.v = 50; // Value from PDF pg 44
    }
...
function getLockDifficulty(lootFocus) {
    ...
    if (lootFocus === "HIGH_VALUE") {
        ...
        if (roll < 0.35) return "HARD (4 Successes)";
        if (roll < 0.70) return "VERY HARD (5 Successes)";
        return "AVERAGE (2 Successes)"; // 30% chance for average
    }
    if (lootFocus === "MEDS" || lootFocus === "AMMO_EXP") {
        // Rule B: MEDS/AMMO -> EASY or AVERAGE (70% chance)
        if (roll < 0.35) return "EASY (1 Success)";
        if (roll < 0.70) return "AVERAGE (2 Successes)";
        return "HARD (4 Successes)"; // 30% chance for hard
    }
    // Default (JUNK/SURVIVAL/PAPER) -> mostly EASY, often unlocked (handled by lock chance)
    if (roll < 0.70) return "EASY (1 Success)";
    return "AVERAGE (2 Successes)";
}
// ----------------------------------------------------------------------
...
const ITEM_DATABASE = {
    ...
    // Sources: PDF Pg 36 (Armor), 40-41 (Energy/Big Guns), 43-44 (Medical/Junk)
    // -------------------------------------------------------------------------
    vault: [
        // Junk & Utility
        ...
        {n: "Pip-Boy 3000", v: 1000}, // Rare find
        ...
        {n: "Alien Biogel", v: 25}, // Rare

        // Weapons
        {n: "Laser Pistol", v: 200}, {n: "10mm Pistol", v: 250}, {n: "Police Baton", v: 25},
        {n: "Ripper", v: 100}, {n: "Plasma Rifle", v: 1200}, {n: "Mesmetron", v: 500},
        {n: "Shock Baton", v: 70}, {n: "Pulse Grenade", v: 60}
    ],

    // -------------------------------------------------------------------------
    // RUINS: Guns, Ammo, Scavenged Armor, Alcohol, Industrial Trash
    // Sources: PDF Pg 39-40 (Small/Big Guns), 43 (Ammo/Junk), 45 (Chems/Booze)
    // -------------------------------------------------------------------------
    ruins: [
        // Junk & Food
        ...
        {n: "Mini-Nuke", v: 250}, {n: "Missile", v: 50},
    ...
    // -------------------------------------------------------------------------
    // CAVE: Tribal Gear, Animal Parts, Primitive Weapons, Survival Items
    // Sources: PDF Pg 38 (Tribal), 43 (Food/Meat), 42 (Melee)
    // -------------------------------------------------------------------------
    cave: [
        // Natural & Survival
    ...
    if (data.loot) for(let item of data.loot) {
       ...
       const typeMap = { 
        ...
        if (item.looted) {
            ...
            // 💡 SPECIAL GLOW: Roentgen Rum & Nuka-Cola Quantum (V's Pro-Tip!)
            if (item.contents && item.contents.some(c => c.n.includes("Roentgen") || c.n.includes("Quantum"))) {
                const hasQuantum = item.contents.some(c => c.n.includes("Quantum"));
                const glowColor = hasQuantum ? 'rgba(0, 150, 255, 0.8)' : 'rgba(0, 255, 100, 0.8)'; // Blue for Quantum, Green for Roentgen
                ...
                const glowGrad = ctx.createRadialGradient(px+gs/2, py+gs/2, 0, px+gs/2, py+gs/2, gs*1.2);
                ...
                ctx.beginPath();
                ctx.arc(px+gs/2, py+gs/2, gs*1.2, 0, Math.PI*2);
                ctx.fill();
