...
function drawCurrentLevel(time = 0) {
    ...
    if (config.showLabels) {
        // ✨ ENHANCED READABILITY: Larger font, better contrast, stronger shadows
        ctx.font = "bold 18px 'VT323', monospace"; // Increased from 14px
        ctx.textAlign = "center"; 
        ...
        for (let lbl of data.labels) {
            ...
            const txtW = ctx.measureText(lbl.text).width + 16; // Extra padding
            const txtH = 26; // Taller boxes
            const lx = lbl.x; 
            const ly = lbl.y;	
            ...
            // Bright, single-color text (no chromatic aberration - clearer!)
            ctx.fillStyle = isInteractive ? '#93c5fd' : '#f1f5f9';
    ...
    for (let t of tokens) {
        ...
        // Draw Label
        ctx.fillStyle = '#fff';
                ctx.font = `bold ${20 * RENDER_SCALE}px monospace`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
