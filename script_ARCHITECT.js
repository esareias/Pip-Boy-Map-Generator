...
const PREFABS = {
    "MEDICAL_WING": {
        ...
        rooms: [
            { name: "ER Waiting Room", w: 6, h: 6, dx: 0, dy: 0 }, 
            { name: "Triage Center", w: 5, h: 5, dx: 6, dy: 1 },       
            { name: "Morgue", w: 4, h: 4, dx: -4, dy: 1 }         
        ]
    },
    "OVERSEER_SUITE": {
        tags: ["VAULT"],
        rooms: [
            { name: "Overseer's Office", w: 8, h: 8, dx: 0, dy: 0 },
            { name: "Private Quarters", w: 5, h: 5, dx: 0, dy: -5 }, 
            { name: "Secret Escape Tunnel", w: 2, h: 6, dx: 8, dy: 1 } 
        ]
    },
    "SCHOOL_WING": {
        tags: ["INSTITUTIONAL"],
        rooms: [
            { name: "Main Hall", w: 4, h: 10, dx: 0, dy: 0 },
            { name: "Classroom 101", w: 5, h: 5, dx: 4, dy: 0 },
            { name: "Classroom 102", w: 5, h: 5, dx: 4, dy: 5 },
            { name: "Principal's Office", w: 4, h: 4, dx: -4, dy: 0 }
        ]
    },
    "FACTORY_LINE": {
        tags: ["INDUSTRIAL"],
        rooms: [
            { name: "Assembly Floor", w: 10, h: 6, dx: 0, dy: 0 },
            { name: "Loading Dock", w: 6, h: 6, dx: 10, dy: 0 },
            { name: "Foreman's Office", w: 4, h: 4, dx: 0, dy: -4 }
        ]
    }
};
...
// Helper to place a cluster of rooms (PREFAB LOGIC)
function tryPlacePrefab(data, gridConfig, archetypeKey) {
    // Find a prefab that matches the current theme/archetype
    const candidates = Object.values(PREFABS).filter(p => p.tags.includes(archetypeKey));
    if (candidates.length === 0) return false;

    const prefab = candidates[Math.floor(Math.random() * candidates.length)];

    // Find a spot for the "Anchor" (First room in prefab)
    const anchorX = Math.floor(Math.random() * (gridConfig.cols - 20)) + 10;
    const anchorY = Math.floor(Math.random() * (gridConfig.rows - 20)) + 10;

    // Check collision for ALL rooms in cluster first
    let canPlace = true;
    for (let r of prefab.rooms) {
        const rx = anchorX + r.dx;
        const ry = anchorY + r.dy;
        // Boundary check
        if (rx < 2 || ry < 2 || rx + r.w > gridConfig.cols - 2 || ry + r.h > gridConfig.rows - 2) {
            canPlace = false; break;
        }
        // Collision check (simplified)
        if (data.grid[rx][ry] === 1) { canPlace = false; break; }
    }

    if (canPlace) {
        log(`PREFAB DEPLOYED: ${prefab.tags[0]} WING`, 'var(--pip-amber)');
        for (let r of prefab.rooms) {
            const roomObj = { 
                x: anchorX + r.dx, 
                y: anchorY + r.dy, 
            ...
            createRoom(data.grid, roomObj, gridConfig);
            data.rooms.push(roomObj);
            addLabelToData(data, roomObj.x + Math.floor(r.w/2), roomObj.y + Math.floor(r.h/2), r.name);

            // Ensure connections between prefab parts
            if (r !== prefab.rooms[0]) {
                // Connect back to anchor
                createCorridor(data.grid, 
                    anchorX + Math.floor(prefab.rooms[0].w/2), 
                    anchorY + Math.floor(prefab.rooms[0].h/2), 
                    roomObj.x + Math.floor(roomObj.w/2), 
                    roomObj.y + Math.floor(roomObj.h/2), 
                    gridConfig
                );
            }
        }
...
function generateInterior(key, name) {
    ...
    // Try to place a prefab structure first (30% chance)
    if (Math.random() < 0.3) {
        tryPlacePrefab(newData, intConfig, archKey);
    }
...
function generateDecorations(data, type, density) {
    ...
    // --- HAZARD GENERATION (ARCHITECT UPGRADE) ---
    for(let x=1; x<config.cols-1; x++) {
        for(let y=1; y<config.rows-1; y++) {
            if (data.grid[x][y] === 1 && Math.random() < 0.015 && !isOccupied(data, x, y)) {
                // Identify room type for context
                const room = data.rooms ? data.rooms.find(r => x >= r.x && x < r.x + r.w && y >= r.y && y < r.y + r.h) : null;
                const roomName = room ? room.name.toUpperCase() : "";

                if (roomName.includes("REACTOR") || roomName.includes("BOILER") || roomName.includes("KITCHEN")) {
                    data.decorations.push({x, y, type: "steam_vent"});
                } 
                else if (roomName.includes("CAVE") || roomName.includes("SEWER") || roomName.includes("LAB")) {
                    data.decorations.push({x, y, type: "toxic_sludge"});
                } 
                else if (roomName.includes("HALL") || roomName.includes("CORRIDOR")) {
                    data.decorations.push({x, y, type: "bear_trap"});
                }
            }
        }
    }
...
function createRoom(grid, room, conf) {
    // 1. Draw Main Body
    for (let x = room.x; x < room.x + room.w; x++) {
        for (let y = room.y; y < room.y + room.h; y++) {
            if (x < conf.cols - 1 && y < conf.rows - 1 && x > 0 && y > 0) grid[x][y] = 1;
        }
    }

    // 2. 40% Chance to add a "Wing" (L-Shape) if it's not a tiny room
    if (room.w > 4 && room.h > 4 && Math.random() < 0.4) {
        const wingW = Math.floor(Math.random() * (room.w - 2)) + 2;
        const wingH = Math.floor(Math.random() * (room.h - 2)) + 2;

        // Determine direction (Right or Down extension)
        if (Math.random() < 0.5) {
            // Extend Right from bottom half
            const startY = room.y + room.h - wingH;
            for (let x = room.x + room.w; x < room.x + room.w + 3; x++) { // Extend 3 tiles out
                for (let y = startY; y < startY + wingH; y++) {
                    if (x < conf.cols - 1 && y < conf.rows - 1) grid[x][y] = 1;
                }
            }
        } else {
            // Extend Down from right side
            const startX = room.x + room.w - wingW;
            for (let x = startX; x < startX + wingW; x++) {
                for (let y = room.y + room.h; y < room.y + room.h + 3; y++) { // Extend 3 tiles down
                    if (x < conf.cols - 1 && y < conf.rows - 1) grid[x][y] = 1;
                }
            }
        }
...
function drawSprite(ctx, type, x, y, size, time) {
    ...
    // --- HAZARD RENDERING (ARCHITECT UPGRADE) ---
    else if (type === 'steam_vent') {
        // Hissing Steam
        ctx.fillStyle = 'rgba(200, 200, 200, 0.3)';
        ctx.beginPath(); ctx.arc(cx, cy, size*0.4, 0, Math.PI*2); ctx.fill();
        // Animated Puffs
        const puff = (time / 200) % 10;
        ctx.fillStyle = `rgba(255, 255, 255, ${0.8 - (puff/10)})`;
        ctx.beginPath(); ctx.arc(cx, cy - puff, size*(0.2 + puff/20), 0, Math.PI*2); ctx.fill();
    }
    else if (type === 'toxic_sludge') {
        // Bubbling Green Goo
        ctx.fillStyle = '#3f6212'; // Dark Green Base
        ctx.beginPath(); 
        ctx.ellipse(cx, cy, size*0.4, size*0.25, 0, 0, Math.PI*2); 
        ctx.fill();

        // Animated Bubble
        if (Math.sin(time/300 + x) > 0.5) {
            ctx.fillStyle = '#a3e635'; // Bright Green Bubble
            ctx.beginPath(); ctx.arc(cx + (Math.sin(time)*4), cy - 2, 3, 0, Math.PI*2); ctx.fill();
        }
    }
    else if (type === 'bear_trap') {
        // Rusty Metal Jaws
        ctx.strokeStyle = '#451a03'; // Rust
        ctx.lineWidth = 2;
