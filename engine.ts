import { FloorData, Token, Room, ViewMode, LootContainer, Item, Decoration } from '../types';
import { BUILDING_ARCHETYPES, ROOM_RELATIONS, PALETTES, ITEM_DATABASE, CONTAINER_DETAILS, DECO_POOLS, SUB_THEMES, NAMES, NON_ENTERABLE } from '../constants';
...
export class GameEngine {
  ...
  floorData: Record<number, FloorData> = {};
  interiorData: Record<string, FloorData> = {};
  tokens: Token[] = [];
  
  currentLevelIndex: number = 0;
  viewMode: ViewMode = 'sector';
  currentInteriorKey: string | null = null;
  mapType: string = 'vault';
  fogEnabled: boolean = true;
  showLabels: boolean = true;
  
  // Rendering internals
  RENDER_SCALE: number = 2;
  mapOffsetX: number = 0;
  mapOffsetY: number = 0;
  patternCache: Record<string, CanvasPattern> = {};
  cloudCanvas: HTMLCanvasElement | null = null;
  tumbleweeds: any[] = [];
  dustMotes: any[] = [];
  
  // Callbacks
  onLog: (msg: string, color?: string, sender?: string) => void;
  onSync: () => void;
  ...
  constructor(canvas: HTMLCanvasElement, onLog: any, onSync: any) {
    this.canvas = canvas;
  ...
  changeLevel(delta: number) {
    ...
    // Notify UI? handled by onSync mostly or wrapper
    this.onSync();
  }

  generateCurrentLevel(density: number) {
      this.patternCache = {};
      ...
      this.onLog(`SCAN COMPLETE: LEVEL ${this.currentLevelIndex}`, 'var(--pip-color)');
      this.onSync();
  }
  
  createRoom(grid: number[][], r: any) {
      for(let x=r.x; x<r.x+r.w; x++) for(let y=r.y; y<r.y+r.h; y++) {
          if (x < this.cols && y < this.rows) grid[x][y] = 1;
      }
  }
  
  createCorridor(grid: number[][], x1: number, y1: number, x2: number, y2: number) {
      let x = x1; let y = y1;
      while (x !== x2) {
  ...
  getRoomDecision(archetypeKey: string, currentRooms: any[], sourceRoomName: string | null) {
      ...
      if (archetypeKey === 'VAULT' && this.currentLevelIndex !== 0) {
          effectiveMandatory = effectiveMandatory.filter((m: string) => m !== "Entrance Airlock");
      }

      const unbuiltMandatory = effectiveMandatory.filter((m: string) => !currentRooms.some(r => r.name === m));
      ...
      candidates = candidates.filter(c => {
        if (arch.unique && arch.unique.includes(c)) {
            ...
            if (archetypeKey === 'VAULT') {
                ...
                const allRooms = Object.values(this.floorData).reduce((arr: any[], lvl: FloorData) => {
                ...
                return !allRooms.some((r: any) => r.name === c);
            } else {
  ...
  generateVault(data: FloorData, density: number, anchors: any[]) {
  ...
  generateRuins(data: FloorData, density: number, anchors: any[]) {
      const buildings: any[] = [];
      const BUFFER = 3;
      
      if (this.currentLevelIndex < 0) {
          // INTERIOR LOGIC (Level -1, -2)
          data.grid = Array(this.cols).fill(0).map(() => Array(this.rows).fill(0));
          
          // Smart Theme Selection based on anchors
        ...
        while (placed < numBuildings && attempts < 1000) {
            ...
            if(!failed) {
                // In Ruins mode, buildings are 0 (Obstacles/Walls)
  ...
  generateCaves(data: FloorData, density: number, anchors: any[]) {
  ...
  generateLocksAndKeys(data: FloorData) {
    ...
    for (let r of data.rooms) {
        ...
        if (isLocked && availableColors.length > 0) {
            const door = data.doors.find(d => {
                // Approximate parent check by proximity
  ...
  pickWeightedItem(type: string, requiredFocus: string | null = null, isLockedContainer: boolean = false) {
    const pool = ITEM_DATABASE[type] || ITEM_DATABASE['cave'];
    let effectivePool = pool;
    
    if (requiredFocus) {
        // Simple filter based on string match logic from constants
    ...
    const getWeight = (itemValue: number) => {
  ...
  generateLoot(data: FloorData, type: string) {
  ...
  generateDecorations(data: FloorData, type: string, density: number) {
  ...
  createPixelPattern(colors: any, type: string) {
  ...
  drawSprite(ctx: CanvasRenderingContext2D, type: string, x: number, y: number, size: number, time: number) {
  ...
  draw(time: number) {
      ...
      // Flashlight effect
      this.ctx.restore(); // Restore to screen coords
      
      this.ctx.save();
      // Volumetric dark overlay
      this.ctx.fillStyle = 'rgba(0,0,0,0.3)';
      this.ctx.fillRect(0,0, this.width*this.RENDER_SCALE, this.height*this.RENDER_SCALE);
      
      // Flashlight hole
      const lx = this.lastPanX; 
      // Restoring dust
      ...
      // UI Layer (Tokens & Labels) - Draw on top
      this.ctx.save();
      // Need to re-apply transform for labels but NOT for static UI. 
      // Tokens move with map, so apply transform.
      this.ctx.scale(this.RENDER_SCALE, this.RENDER_SCALE);
      this.ctx.translate(this.mapOffsetX, this.mapOffsetY);
