...
// ========================================
// FIRST-PERSON MODE - KEYBOARD & FUNCTIONS
// ========================================
...
function toggleFirstPersonMode() {
  ...
  if (firstPersonMode) {
    ...
    log('W/S=MOVE, A/D=STRAFE, Q/E=TURN, V=TOGGLE', 'var(--pip-amber)');
...
function updateFirstPersonMovement() {
  ...
  let moveX = 0, moveY = 0;
  if (moveForward) { moveX += Math.cos(playerAngle) * playerSpeed; moveY += Math.sin(playerAngle) * playerSpeed; }
  if (moveBackward) { moveX -= Math.cos(playerAngle) * playerSpeed; moveY -= Math.sin(playerAngle) * playerSpeed; }
  if (strafeLeft) { moveX += Math.cos(playerAngle - Math.PI/2) * playerSpeed; moveY += Math.sin(playerAngle - Math.PI/2) * playerSpeed; }
  if (strafeRight) { moveX += Math.cos(playerAngle + Math.PI/2) * playerSpeed; moveY += Math.sin(playerAngle + Math.PI/2) * playerSpeed; }
...
function castRay(data, startX, startY, angle) {
  ...
  while (distance < maxDistance) {
    ...
    if (gridX < 0 || gridX >= grid.length || gridY < 0 || gridY >= grid[0].length) {
      return { distance: distance, hit: true, gridX, gridY };
    }
...
function drawFirstPerson() {
  ...
  // Get palette matching top-down view
  ...
  // Ceiling and floor using palette colors
  ctx.fillStyle = pal.bg;
  ctx.fillRect(0, 0, canvas.width, canvas.height / 2);
  ctx.fillStyle = pal.floor.base;
  ...
  for (let i = 0; i < rayCount; i++) {
    ...
    if (hit.hit) {
      ...
      const distanceFactor = Math.max(0, Math.min(1, 1 - (correctedDistance / 30)));

      // Choose wall color based on orientation
      const isVertical = Math.abs(Math.cos(rayAngle)) > Math.abs(Math.sin(rayAngle));
      const baseColor = isVertical ? pal.wall.front : pal.wall.top;
      const rgb = hexToRgb(baseColor);

      const r = Math.floor(rgb.r * distanceFactor);
      const g = Math.floor(rgb.g * distanceFactor);
      const b = Math.floor(rgb.b * distanceFactor);
  ...
  // Floor gradient with palette colors
  for (let y = Math.floor(halfHeight); y < screenHeight; y++) {
    const distance = (screenHeight / (2 * y - screenHeight)) * 5;
    const distanceFactor = Math.max(0, Math.min(1, 1 - (distance / 25)));
    const floorRgb = hexToRgb(pal.floor.base);
    const darkRgb = hexToRgb(pal.floor.dark);
    const r = Math.floor((floorRgb.r * 0.7 + darkRgb.r * 0.3) * distanceFactor);
    const g = Math.floor((floorRgb.g * 0.7 + darkRgb.g * 0.3) * distanceFactor);
    const b = Math.floor((floorRgb.b * 0.7 + darkRgb.b * 0.3) * distanceFactor);
    ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
  ...
  ctx.strokeStyle = pal.accent || '#0f0';
  ctx.lineWidth = 3;
  ctx.beginPath();
