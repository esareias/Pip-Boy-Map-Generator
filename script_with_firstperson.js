...
// ========================================
let firstPersonMode = false; // Toggle between top-down and first-person
let playerAngle = 0; // Player rotation in radians
let playerSpeed = 0.5; // Movement speed per grid unit
let rotationSpeed = 0.05; // Rotation speed
let fov = Math.PI / 3; // 60 degree field of view
let rayCount = 320; // Number of rays to cast (resolution)
...
// ========================================
// FIRST-PERSON RAYCASTING FUNCTIONS
// ========================================

function updateFirstPersonMovement() {
  if (!firstPersonMode) return;

  // Find player token (use first non-Overseer token or any token)
  ...
  // Only move if new position is walkable (grid value 1 = floor)
...
function castRay(data, startX, startY, angle) {
  ...
  if (!grid) return { distance: 1000, wallType: 0 };
  ...
  while (distance < maxDistance) {
    ...
    if (gridX < 0 || gridX >= grid.length || gridY < 0 || gridY >= grid[0].length) {
      return { distance: distance, wallType: 1, gridX, gridY };
    }

    // Check for wall (0 = wall, 1 = floor)
    if (grid[gridX][gridY] === 0) {
      return { distance: distance, wallType: 1, gridX, gridY };
    }
  }

  return { distance: maxDistance, wallType: 0 };
}
...
function drawFirstPerson() {
  ...
  if (!data || !data.grid) {
    ...
    ctx.fillText('NO MAP DATA - SCAN LEVEL FIRST', canvas.width/2 - 150, canvas.height/2);
    return;
  }
  ...
  if (!player) {
    ...
    ctx.fillStyle = '#f00';
    ctx.font = '20px monospace';
    ctx.fillText('NO PLAYER TOKEN - DEPLOY UNIT FIRST', canvas.width/2 - 180, canvas.height/2);
    return;
  }

  const screenWidth = canvas.width / RENDER_SCALE;
  const screenHeight = canvas.height / RENDER_SCALE;
  ...
  ctx.fillText('FIRST-PERSON MODE', 10, 30);
  ctx.fillText(`ANGLE: ${Math.round(playerAngle * 180 / Math.PI)}°`, 10, 50);
  ctx.fillText(`POS: ${Math.floor(player.x)}, ${Math.floor(player.y)}`, 10, 70);
  ctx.fillText('[W/S] MOVE [A/D] STRAFE [Q/E] TURN [V] TOGGLE VIEW', 10, canvas.height - 10);
...
function toggleFirstPersonMode() {
  ...
  if (firstPersonMode) {
    ...
    log('USE W/A/S/D TO MOVE, Q/E OR ARROWS TO TURN', 'var(--pip-amber)');
  } else {
...
function drawCurrentLevel(time = 0) {
  ...
  // TOP-DOWN MODE (original rendering below)
