...
function drawCRTEffects(ctx, width, height) {
    ...
    // 📺 ANIMATED SCANLINES (Retro terminal feel)
...
function drawCurrentLevel(time = 0) {
    ...
    // --- SMOOTH CAVE RENDERING (Polished Natural Look) ---
    if (patternType === 'cave') {
        ...
        for (let x = 0; x < config.cols; x++) {
            for (let y = 0; y < config.rows; y++) {
                if (data.grid[x][y] >= 1) {
                    ...
                    const n = (y > 0 && data.grid[x][y-1] >= 1);
                    const s = (y < config.rows-1 && data.grid[x][y+1] >= 1);
                    const e = (x < config.cols-1 && data.grid[x+1][y] >= 1);
                    const w = (x > 0 && data.grid[x-1][y] >= 1);

                    // Smooth rounded tile
                    const r = gs * 0.15; // Corner radius
                    ctx.beginPath();
                    ctx.moveTo(px + r, py);

                    // Top right corner
                    ctx.lineTo(px + gs - r, py);
                    if (n && e) ctx.lineTo(px + gs, py);
                    else ctx.quadraticCurveTo(px + gs, py, px + gs, py + r);

                    // Bottom right corner
                    ctx.lineTo(px + gs, py + gs - r);
                    if (s && e) ctx.lineTo(px + gs, py + gs);
                    else ctx.quadraticCurveTo(px + gs, py + gs, px + gs - r, py + gs);

                    // Bottom left corner
                    ctx.lineTo(px + r, py + gs);
                    if (s && w) ctx.lineTo(px, py + gs);
                    else ctx.quadraticCurveTo(px, py + gs, px, py + gs - r);

                    // Top left corner
                    ctx.lineTo(px, py + r);
                    if (n && w) ctx.lineTo(px, py);
                    else ctx.quadraticCurveTo(px, py, px + r, py);
                    ...
                    if ((x + y) % 4 === 0) {
                        ...
                        ctx.fillRect(px + gs*0.3, py + gs*0.5, gs*0.4, 1);
                        ctx.globalAlpha = 1.0;
                        ctx.fillStyle = floorPattern;
                    }
                    ...
                    if (data.grid[x][y] === 2) {
                        ...
                        ctx.fill();
                        const pulse = Math.sin(time / 400 + x + y) * 0.3 + 0.4;
                        ctx.fillStyle = `rgba(132, 204, 22, ${pulse * 0.5})`;
                        ctx.beginPath();
                        ctx.arc(px + gs/2, py + gs/2, gs * 0.35, 0, Math.PI * 2);
                        ctx.fill();
                        ctx.fillStyle = floorPattern;
                    }
                }
    ...
    if (data.loot) for(let item of data.loot) {
       ...
       const typeMap = { 
        ...
        if (item.looted) {
            ...
            // ✨ Soft pulsing glow for unlocked containers
            if (!item.looted && !item.isLocked) {
                const glowPulse = Math.sin(time / 400) * 0.15 + 0.3;
                const glowGrad = ctx.createRadialGradient(px+gs/2, py+gs/2, 0, px+gs/2, py+gs/2, gs*0.8);
                glowGrad.addColorStop(0, `rgba(255, 220, 100, ${glowPulse})`);
                glowGrad.addColorStop(1, 'rgba(255, 220, 100, 0)');
                ctx.fillStyle = glowGrad;
                ctx.fillRect(px - gs*0.2, py - gs*0.2, gs*1.4, gs*1.4);
            }
            ctx.globalCompositeOperation = 'lighter';
    ...
    // --- LIGHTING ENGINE (Volumetric Pass) ---
        // Dynamic shadow intensity based on environment
    let shadowDepth = 0.35;
    if (config.mapType === 'cave') {
        shadowDepth = currentLevelIndex < 0 ? 0.55 : 0.40; // Darker underground
    }
    ...
    // 💡 Enhanced flashlight with soft falloff
    ...
    // Enhanced flashlight with layered glow
    const cursorG = ctx.createRadialGradient(lightX, lightY, 0, lightX, lightY, 250);
    cursorG.addColorStop(0, 'rgba(255, 255, 255, 0.20)');
    cursorG.addColorStop(0.3, 'rgba(255, 255, 200, 0.12)');
    cursorG.addColorStop(0.7, 'rgba(200, 255, 200, 0.06)');
    cursorG.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = cursorG;
    ctx.fillRect(0, 0, config.width, config.height);

    // ✨ Smooth dust particles with glow
    for(let m of dustMotes) {
        ...
        if (dist < 240) {
            const visibility = 1 - (dist / 240);

            // Soft glowing dust particle
            const dustGlow = ctx.createRadialGradient(m.x, m.y, 0, m.x, m.y, m.size * 2);
            dustGlow.addColorStop(0, `rgba(220, 255, 220, ${visibility * 0.6})`);
            dustGlow.addColorStop(0.5, `rgba(200, 255, 200, ${visibility * 0.3})`);
    ...
    if (data.decorations) for (let deco of data.decorations) {
        ...
        if (deco.type === 'fire_barrel' || deco.type === 'campfire') {
                        const flicker = Math.sin(time / 80) * 8 + Math.random() * 6;
                        radius = gs * 3.5 + flicker;
    ...
    // 🏜️ Warm desert color tint (cave only)
    if (config.mapType === 'cave') {
        ctx.globalCompositeOperation = 'overlay';
        const warmGrad = ctx.createRadialGradient(
            config.width/2, config.height/2, 0,
            config.width/2, config.height/2, config.width * 0.75
        );
        warmGrad.addColorStop(0, 'rgba(255, 210, 140, 0.12)');
        warmGrad.addColorStop(0.6, 'rgba(240, 190, 130, 0.08)');
        warmGrad.addColorStop(1, 'rgba(200, 160, 120, 0.04)');
